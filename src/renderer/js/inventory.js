        ...
        let nextStockId = stockItems.length + 1;
        ...
        function getExpiryStatus(daysLeft) {
            if (daysLeft < 0) return { status: "Expired", class: "expired", icon: "âŒ" };
            if (daysLeft <= 30) return { status: "Expiring Soon", class: "expiring", icon: "âš ï¸" };
            return { status: "Good", class: "good", icon: "âœ…" };
        }
        ...
        function addStock() {
            ...
            renderStockTable();
            alert(`âœ… Added ${quantity} Ã— ${product.name} to inventory!`);
        }
        ...
        function renderStockTable() {
            ...
            if (filteredItems.length === 0) {
                tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state">ðŸ“¦ No stock items found</div></td></tr>`;
                return;
            }
            ...
            tbody.innerHTML = filteredItems.map(item => {
                ...
                return `
                    <tr style="${isExpired ? 'opacity: 0.6; background-color: #FEE2E2;' : ''}">
